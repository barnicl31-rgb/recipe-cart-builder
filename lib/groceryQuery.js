const catalog = [
  ...
  entry("cream", ["heavy cream", "cooking cream", "whipping cream"]),
...
function buildGrocerySearchQueries(ingredient) {
  ...
  for (const value of values) {
    ...
    const fallback = fallbackTerms.get(query);
...
function canonicalizeGroceryQuery(value) {
  let phrase = normalizeText(value)
    ...
    .replace(/^\s*(?:cups?|tablespoons?|tbsp|teaspoons?|tsp|grams?|g|kilograms?|kg|milliliters?|ml|liters?|l|ounces?|oz|pounds?|lbs?|pinches?|cans?|packets?|pieces?|slices?)\b\s*/i, "")
